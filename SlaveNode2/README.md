# ModMesh: Slave Node 2 Documentation

## 📌 Introduction

The **ModMesh Slave Node 2** operates as a wireless bridge sensor node in the ModMesh ecosystem. It is an **Encrypted Modbus RTU Slave Router** that acts as the Modbus Master to physical RS-485 sensors (such as temp/humidity sensors, meters, etc.) and routes requests back and forth to the **Master Node** over the air using the **ESP-NOW** protocol.

It is paired specifically to handle queries meant for **Slave ID 2** and communicates directly with the Master Node.
...
1. **Master Node** sends an encrypted Modbus query meant for Slave ID 2 over ESP-NOW.
...
*Developed by M. YOUCEF Yazid | v1.0.0 Slave Node 2 Edition*
